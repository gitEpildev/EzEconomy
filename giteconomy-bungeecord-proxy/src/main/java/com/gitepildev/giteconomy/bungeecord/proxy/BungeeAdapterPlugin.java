package com.gitepildev.giteconomy.bungeecord.proxy;
...
public class BungeeAdapterPlugin {
    private final GitBungeeProxy proxyLogic;
    // temporary holder used when parsing config externally
    private String _tmpSecret = "";

    public BungeeAdapterPlugin() {
        this.proxyLogic = new GitBungeeProxy(_tmpSecret);
    }

    /**
     * Create an `GitBungeeProxy` configured from a `bungeecord.yml` file. The
     ...
     */
    public static GitBungeeProxy loadProxyFromConfig(java.io.File configFile) {
        ...
        return new GitBungeeProxy(secretRef.get(), cleanupRef.get());
    }
    ...
    public void tryEnable() {
        try {
            ...
            // provided by `GitBungeeProxyPlugin` when running on a proxy. This
    ...
    public GitBungeeProxy getProxyLogic() { return proxyLogic; }
}

package com.gitepildev.giteconomy.cache;
        ...
        try {
            switch (strategy) {
                case REDIS:
                    ...
                    try {
                        Class<?> c = Class.forName("com.gitepildev.giteconomy.redis.cache.RedisCacheProvider");
                ...
                case BUNGEECORD:
                    try {
                        Class<?> c = Class.forName("com.gitepildev.giteconomy.bungeecord.cache.BungeeCacheProvider");
                        provider = (CacheProvider<?, ?>) c.getDeclaredConstructor().newInstance();
                        return;
                    } catch (ClassNotFoundException ignored) {}
                ...
                case DATABASE:
                    try {
                        Class<?> c = Class.forName("com.gitepildev.giteconomy.storage.cache.DatabaseCacheProvider");
                        provider = (CacheProvider<?, ?>) c.getDeclaredConstructor().newInstance();
                        return;
                    } catch (ClassNotFoundException ignored) {}

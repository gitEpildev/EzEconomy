...
public class CurrencyCommand implements CommandExecutor {
	...
	public CurrencyCommand(GitEconomyPlugin plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args[0].equalsIgnoreCase("convert")) {
			...
			plugin.getTransactionMetricsService().logTransaction(new com.gitepildev.giteconomy.api.storage.models.Transaction(player.getUniqueId(), from, -usedSource.doubleValue(), now));
			plugin.getTransactionMetricsService().logTransaction(new com.gitepildev.giteconomy.api.storage.models.Transaction(player.getUniqueId(), to, res.converted.doubleValue(), now));

...
import com.gitepildev.giteconomy.lock.LockManager;
import com.gitepildev.giteconomy.lock.LocalLockManager;
import java.net.URL;
...
public class LockingComponent implements BootstrapComponent {
    ...
    public LockingComponent(GitEconomyPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public void start() {
        ...
        if ("BUNGEECORD".equals(strategy) && bungeeCfg.getBoolean("enabled", false)) {
            ...
            try {
                ...
                if (this.manager == null) {
                    try {
                        Class<?> clazz = Class.forName("com.gitepildev.giteconomy.lock.BungeeLockManager");
                        ...
                        try {
                            Class<?> transportClazz = Class.forName("com.gitepildev.giteconomy.lock.transport.PluginMessagingTransport");
                            ...
                            try {
                                ...
                                transport = transportClazz.getConstructor(org.bukkit.plugin.Plugin.class, String.class, long.class, String.class)
                                        .newInstance(plugin, bungeeCfg.getString("channel", "giteconomy:locks"), bungeeCfg.getLong("ttl-ms", 60000L), bungeeCfg.getString("shared-secret", ""));
                                ...
                                transport = transportClazz.getConstructor(org.bukkit.plugin.Plugin.class, String.class, long.class)
                                        .newInstance(plugin, bungeeCfg.getString("channel", "giteconomy:locks"), bungeeCfg.getLong("ttl-ms", 60000L));
                            }
                            // call BungeeLockManager.setGlobalTransport(transport)
                            Class<?> bmClazz = Class.forName("com.gitepildev.giteconomy.lock.BungeeLockManager");
                            java.lang.reflect.Method setGlobal = bmClazz.getMethod("setGlobalTransport", Class.forName("com.gitepildev.giteconomy.lock.transport.LockTransport"));
            ...
            try {
                ...
                if (this.manager == null) {
                    try {
                        Class<?> clazz = Class.forName("com.gitepildev.giteconomy.lock.RedisLockManager");

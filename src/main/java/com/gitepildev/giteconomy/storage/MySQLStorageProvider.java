...
/**
 * MySQL implementation of the StorageProvider interface for GitEconomy.
...
public class MySQLStorageProvider implements StorageProvider {
    ...
    public MySQLStorageProvider(GitEconomyPlugin plugin, YamlConfiguration dbConfig) {
        this.plugin = plugin;
    ...
    public void load() throws StorageLoadException {
        ...
        config.setPoolName("GitEconomy-HikariPool");
    ...
    public java.util.List<Transaction> getTransactions(java.util.UUID uuid, String currency) {
        ...
        try (Connection conn = getConn()) {
            ...
            plugin.getLogger().severe("[GitEconomy] MySQL getTransactions failed for " + uuid + " (" + currency + "): " + e.getMessage());
        }
        return transactions;
    }
    ...
    public double getBalance(UUID uuid, String currency) {
        try (Connection conn = getConn()) {
            ...
            plugin.getLogger().severe("[GitEconomy] MySQL getBalance failed for " + uuid + " (" + currency + "): " + e.getMessage());
        }
        return 0.0;
    }
    ...
    public boolean playerExists(UUID uuid) {
        try (Connection conn = getConn()) {
            ...
            plugin.getLogger().severe("[GitEconomy] MySQL playerExists failed for " + uuid + ": " + e.getMessage());
            return false;
        }
    }
    ...
    public void setBalance(UUID uuid, String currency, double amount) {
        try (Connection conn = getConn()) {
            ...
            plugin.getLogger().severe("[GitEconomy] MySQL setBalance failed for " + uuid + " (" + currency + "): " + e.getMessage());
        }
    }
    ...
    public boolean tryWithdraw(UUID uuid, String currency, double amount) {
        try (Connection conn = getConn()) {
            ...
            plugin.getLogger().severe("[GitEconomy] MySQL tryWithdraw failed for " + uuid + " (" + currency + "): " + e.getMessage());
            return false;
        }
    }
    ...
    public void deposit(UUID uuid, String currency, double amount) {
        try (Connection conn = getConn()) {
            ...
            plugin.getLogger().severe("[GitEconomy] MySQL deposit failed for " + uuid + " (" + currency + "): " + e.getMessage());
        }
    }
    ...
    private void safeUpdatePlayerInfo(Connection conn, UUID uuid) {
        try {
            ...
            plugin.getLogger().warning("[GitEconomy] safeUpdatePlayerInfo failed: " + e.getMessage());
        }
    }
    ...
    public Map<UUID, Double> getAllBalances(String currency) {
        ...
        try (Connection conn = getConn()) {
            ...
            plugin.getLogger().severe("[GitEconomy] MySQL getAllBalances failed (" + currency + "): " + e.getMessage());
        }
        return map;
    }
    ...
    public com.gitepildev.giteconomy.storage.TransferResult transfer(UUID fromUuid, UUID toUuid, String currency, double debitAmount, double creditAmount) {
        ...
        try (Connection conn = getConn()) {
            ...
            try {
                ...
                conn.rollback();
                plugin.getLogger().severe("[GitEconomy] MySQL transfer failed: " + e.getMessage());
                return com.gitepildev.giteconomy.storage.TransferResult.failure(fromBefore, toBefore);
            } finally {
                conn.setAutoCommit(true);
            }
        } catch (SQLException e) {
            plugin.getLogger().severe("[GitEconomy] MySQL transfer connection error: " + e.getMessage());
            return com.gitepildev.giteconomy.storage.TransferResult.failure(fromBefore, toBefore);
        }
    ...
    private void firePreTransaction(UUID from, UUID to, double amount) {
        com.gitepildev.giteconomy.api.events.PreTransactionEvent pre = new com.gitepildev.giteconomy.api.events.PreTransactionEvent(
            from, to, java.math.BigDecimal.valueOf(amount), com.gitepildev.giteconomy.api.events.TransactionType.TRANSFER);
        try {
    ...
    private void firePostTransaction(UUID from, UUID to, double amount, com.gitepildev.giteconomy.storage.TransferResult tr, double fromBefore, double toBefore) {
    ...
    public void insertPendingNotification(UUID uuid, String message) {
        try (Connection conn = getConn()) {
            ...
            plugin.getLogger().warning("[GitEconomy] insertPendingNotification failed: " + e.getMessage());
        }
    }
    ...
    public java.util.List<String> pollPendingNotifications(UUID uuid) {
        ...
        try (Connection conn = getConn()) {
            ...
            plugin.getLogger().warning("[GitEconomy] pollPendingNotifications failed: " + e.getMessage());
        }
        return messages;
    }
    ...
    public void cleanupOldNotifications(long maxAgeMs) {
        try (Connection conn = getConn()) {
            ...
            plugin.getLogger().warning("[GitEconomy] cleanupOldNotifications failed: " + e.getMessage());
        }
    }
    ...
    public UUID resolvePlayerByName(String name) {
        try (Connection conn = getConn()) {
            ...
            plugin.getLogger().warning("[GitEconomy] resolvePlayerByName failed for " + name + ": " + e.getMessage());
        }
        return null;
    }
    ...
    public void persistPlayerInfo(UUID uuid, String name, String displayName) {
        ...
        try (Connection conn = getConn()) {
            ...
            plugin.getLogger().warning("[GitEconomy] persistPlayerInfo failed: " + e.getMessage());
        }
    }
    ...
    public void logTransaction(com.gitepildev.giteconomy.api.storage.models.Transaction tx) {
        try (Connection conn = getConn()) {
            ...
            plugin.getLogger().severe("[GitEconomy] MySQL logTransaction failed: " + e.getMessage());
        }
    }
    ...
    public java.util.Set<String> cleanupOrphanedPlayers() {
        ...
        try (Connection conn = getConn()) {
            ...
            plugin.getLogger().severe("[GitEconomy] MySQL cleanupOrphanedPlayers failed: " + e.getMessage());
        }
        return removed;
    }
    ...
    public java.util.Set<String> previewOrphanedPlayers() {
        ...
        try (Connection conn = getConn()) {
            ...
            plugin.getLogger().severe("[GitEconomy] MySQL previewOrphanedPlayers failed: " + e.getMessage());
        }
        return orphaned;
    }
}

package com.gitepildev.giteconomy.update;
...
public final class SpigotUpdateChecker {
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                ...
                if (isStableVersion(latestVersion) && isNewer(latestVersion, currentVersion)) {
                    plugin.getLogger().info("A new GitEconomy version is available: " + latestVersion
                            + " (current: " + currentVersion + "). Download: https://www.spigotmc.org/resources/"
                            + resourceId + "/");
                } else {
                    plugin.getLogger().info("GitEconomy is up to date.");
                }
            } catch (Exception ex) {
                plugin.getLogger().warning("Failed to check for GitEconomy updates: " + ex.getMessage());
            }
        });
    }
    ...
    private String fetchLatestVersion() throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "GitEconomy Update Checker");

...
import com.gitepildev.giteconomy.storage.TransferLockManager;
import com.gitepildev.giteconomy.storage.TransferResult;
import com.gitepildev.giteconomy.api.events.PlayerPayPlayerEvent;
...
public class PaymentExecutor {
    private static long syncEventTimeoutMs(GitEconomyPlugin plugin) {
     ...
     */
    public static boolean execute(GitEconomyPlugin plugin, Player from, String toName, BigDecimal amountDecimal, String currency) {
        return execute(plugin, from, toName, amountDecimal, currency, false);
    }

    public static boolean execute(GitEconomyPlugin plugin, Player from, String toName, BigDecimal amountDecimal, String currency, boolean knownOffline) {
        ...
        if (online != null) {
            ...
            if (resolvedUuid != null) {
                ...
                var maybe = com.gitepildev.giteconomy.util.PlayerLookup.findByName(toName);
                if (maybe.isPresent()) {
                    toOffline = maybe.get();
                } else {

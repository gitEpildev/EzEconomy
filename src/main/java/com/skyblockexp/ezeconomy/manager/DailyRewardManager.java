...
public class DailyRewardManager {
    ...
    public DailyRewardManager() {
        this.storage = new DailyRewardStorage(com.skyblockexp.ezeconomy.core.Registry.getPlugin());
    }
    ...
    public void handleJoin(Player player) {
        ...
        StorageProvider storageProvider = com.skyblockexp.ezeconomy.core.Registry.get(com.skyblockexp.ezeconomy.api.storage.StorageProvider.class);
        if (storageProvider == null) {
            debug("Storage provider unavailable; skipping daily reward.");
            return;
        }
        double amount = com.skyblockexp.ezeconomy.core.Registry.getPlugin().getConfig().getDouble("daily-reward.amount", 0.0);
        if (amount <= 0.0) {
        ...
        String currency = resolveCurrency(com.skyblockexp.ezeconomy.core.Registry.getPlugin().getConfig().getString("daily-reward.currency", "default"));
    ...
    private boolean isEnabled() {
        return com.skyblockexp.ezeconomy.core.Registry.getPlugin().getConfig().getBoolean("daily-reward.enabled", false);
    }

    private String getPermission() {
        return com.skyblockexp.ezeconomy.core.Registry.getPlugin().getConfig().getString("daily-reward.permission", "ezeconomy.daily");
    }

    private long getCooldownMillis() {
        long hours = com.skyblockexp.ezeconomy.core.Registry.getPlugin().getConfig().getLong("daily-reward.cooldown-hours", 24L);
        if (hours < 0) {
            hours = 0;
        }
    ...
    private String resolveCurrency(String configuredCurrency) {
        String fallback = com.skyblockexp.ezeconomy.core.Registry.get(com.skyblockexp.ezeconomy.manager.CurrencyManager.class).getDefaultCurrency();
        if (configuredCurrency == null || configuredCurrency.trim().isEmpty()
                || configuredCurrency.equalsIgnoreCase("default")) {
            return fallback;
        }
        boolean multiEnabled = com.skyblockexp.ezeconomy.core.Registry.getPlugin().getConfig().getBoolean("multi-currency.enabled", false);
        if (!multiEnabled) {
    ...
    private boolean currencyExists(String currency) {
        ConfigurationSection section = com.skyblockexp.ezeconomy.core.Registry.getPlugin().getConfig()
    ...
    private String resolveCurrencyDisplay(String currency) {
        ...
        String display = com.skyblockexp.ezeconomy.core.Registry.getPlugin().getConfig()
    ...
    private void sendMessage(Player player, double amount, String currency) {
        String messageKey = com.skyblockexp.ezeconomy.core.Registry.getPlugin().getConfig().getString("daily-reward.message-key", "daily_reward_success");
        String formattedAmount = com.skyblockexp.ezeconomy.core.Registry.get(com.skyblockexp.ezeconomy.manager.CurrencyManager.class).format(amount);
        String displayCurrency = resolveCurrencyDisplay(currency);
        MessageUtils.send(player, com.skyblockexp.ezeconomy.core.Registry.getPlugin(), messageKey, Map.of("amount", formattedAmount, "currency", displayCurrency));
    }

    private void playSound(Player player) {
        String soundName = com.skyblockexp.ezeconomy.core.Registry.getPlugin().getConfig().getString("daily-reward.sound", "");

package com.gitepildev.giteconomy.velocity;
...
@Plugin(id = "giteconomy", name = "GitEconomy", version = "2.6.0",
        description = "Cross-server payment forwarding & global player list for GitEconomy",
        authors = {"GitEpildev", "Epildevconnect Ltd"})
public class GitEconomyVelocity {
    private static final MinecraftChannelIdentifier CHANNEL =
            MinecraftChannelIdentifier.create("giteconomy", "notify");

    private final ProxyServer server;
    private final Logger logger;

    @Inject
    public GitEconomyVelocity(ProxyServer server, Logger logger) {
        this.server = server;
        this.logger = logger;
    }
    ...
    public void onProxyInit(ProxyInitializeEvent event) {
        ...
        logger.info("GitEconomy Velocity plugin enabled - registered channel giteconomy:notify");

...
- `REDIS`: Use when multiple server instances should share cache state. Requires the `giteconomy-redis` extension and a working `redis.yml` configuration. Offers low-latency shared cache and eviction/TTL semantics provided by Redis.
- `BUNGEECORD`: Use when the proxy should act as the canonical cache holder. Servers forward cache requests via plugin messaging to the proxy-side store. Requires the proxy component (`giteconomy-bungeecord-proxy`) and `bungeecord.yml` configuration.
- `DATABASE`: Use when you prefer using the database as a cache backend (e.g., small networks without Redis). The plugin stores entries in the `giteconomy_cache` table with TTL metadata; this has higher latency than Redis but requires no extra runtime services.
...
### Redis
1. Install the optional extension `giteconomy-redis.jar` into `plugins/GitEconomy/libs/`.
...
1. Deploy the proxy component `giteconomy-bungeecord-proxy` on your Bungee/Waterfall proxy.
...
2. The plugin will create/use an `giteconomy_cache` table; confirm DB user has appropriate privileges.
